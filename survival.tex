...
\rev{An increasing average temperature gradient of about $4^\circ$C
  per cage is reported in \cite[Fig.10]{tiwari15Understand},
  independently confirming our airflow temperature
  interpretation. They also suggest that DBEs may be sensitive to
  temperature, although consider their evidence preliminary due to
  relatively low number of DBEs in 2014-2015 and high variability of
  $10^\circ$C to $15^\circ$C in their measured cage replicates.}
...
\rev{\cite{Cox1972,Harrell2015}}. The CPH regression function takes
the form
\begin{displaymath}
...
detailed nuances of the system cooling architecture \rev{and are also
  impacted by job scheduling}.  The figures give
...
\rev{due to the presence of the file system to the right of {\em col}
  24, as can be seen in the floor layout of Fig.~\ref{fig:layout}. An
  explanation due to separate supply lines of chilled water to the two
  sets is not likely as the evaporators (see Fig.~\ref{fig:layout})
  are thermostat controlled.} \fix{The file system, however. did not
  have evaporator heat exchangers and relied on ambient room air for
  heat transport, affecting room temperature on the right side of
  Titan. !!Do we have room temp measurement data?!!}

\rev{The peculiar interleaving pattern of the first set of columns can
  be explained by Titan's folded torus interconnect. If we order the
  columns by their torus X coordinate \cite{ezell2013}, seen in
  Fig.~\ref{fig:cox-torus}, the column pattern retains the proximity
  to the file system explanation and loses the interleaving pattern
  peculiarity. Moreover, Titan filled temporal and spatial gaps in
  nodes starting with low torus {\em X} coordinate. This potentially
  explains that the low {\em col} and low {\em X} coordinate columns
  have higher hazard due to higher workload than the low {\em col} and
  high {\em X} coordinate columns.  It is possible that the torus Y
  coordinate can help explain the apparent row effects, but we have
  difficulty interpreting the row description in \cite{ezell2013}.} A
